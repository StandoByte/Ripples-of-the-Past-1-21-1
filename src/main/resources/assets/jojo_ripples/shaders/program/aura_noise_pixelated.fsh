...
uniform int BlurStart = 2;
uniform int BlurRadius = 6;

out vec4 fragColor;

void main(){
	if (texCoord.x < ScreenRatio && texCoord.y < ScreenRatio) {
		vec2 noiseCoord = texCoord / vec2(ScreenRatio);
		vec4 outlineColor = texture(DiffuseSampler, noiseCoord);
		if (outlineColor.a > 0.0) {
			float noise = 1.0;
			
			noise *= 2.0 - voronoise(vec3(noiseCoord * vec2(4.0, 1.0), Time * 4.0), 1.0, 1.0) * 2.0;
			noise *= 1.0 + (fbm(vec3(noiseCoord * vec2(4.0, 1.0), Time * 1.0))) * 2.0;
			
			float alphaThreshold = 0.4;
			if (noise >= alphaThreshold) {
				
				vec4 silhouetteColor = texture(SilhouetteSampler, noiseCoord);
				if (silhouetteColor.a < 1.0) {
					int tooFarDistSqr = 2 * BlurRadius + 1;
					int minDistSqr = tooFarDistSqr;
					for(int x = -BlurRadius; x <= BlurRadius; ++x) {
						for(int y = -BlurRadius; y <= BlurRadius; ++y) {
							int distSqr = x * x + y * y;
							if (distSqr < minDistSqr) {
								vec2 coord = noiseCoord + oneTexel * vec2((x + 0.5) / ScreenRatio, (y + 0.5) / ScreenRatio);
								if(texture(SilhouetteSampler, coord).a > 0.0) {
									minDistSqr = distSqr;
								}
							}
						}
					...
					if (minDistSqr == tooFarDistSqr) {
						noise = 0.0;
					}
					else if (minDistSqr > BlurStart * BlurStart) {
						float alpha = (BlurStart * BlurStart) / float(minDistSqr);
						noise *= alpha;
					}
				}
